...
"both A and B", or their Chinese translations) are left in their original
order, since reordering would break the meaning.

Usage:
    python3 scripts/debias_quizzes.py            # rewrite in place
    python3 scripts/debias_quizzes.py --check     # report only, do not write
"""
import argparse
import collections
import glob
...
ANCHOR = re.compile(
    ...
    r"|\b(?:the|all|none)\s+(?:above|below|following)\b"
    r"|(?:以上|以下|上述|下述)\s*(?:全部|所有)"
    r"|(?:以上|以下|上述|下述).{0,16}(?:都|全部|均|所有).{0,8}(?:正确|错误|成立|不成立)"
    r"|两者.{0,8}(?:都|均|不).{0,8}(?:正确|错误|成立|不成立)"
    r"|(?:[A-D]\s*(?:和|与|、)\s*)+[A-D]\s*(?:都|均|全部)?\s*(?:不\s*)?(?:正确|错误)",
    re.IGNORECASE,
)
